...
class DecantBatch(models.Model):
    ...
    def __str__(self):
        return f"Decant Batch {self.decant_batch_id} - {self.quantity_created}ml"
...
class Invoice(models.Model):
    ...
    status = models.CharField(max_length=9, default='Issued')
...
class OrderItem(models.Model):
    ...
    # MySQL defines this as a GENERATED ALWAYS AS (quantity * unit_price) STORED column.
    # Django 5 GeneratedField prevents ORM INSERTs from trying to write into that column.
...
class Perfume(models.Model):
    ...
    recommended_season = models.CharField(max_length=100, blank=True, null=True)
...
class Product(models.Model):
    ...
    def __str__(self):
        return f"{self.perfume.perfume_name} - {self.volume_ml}ml"
